  ...
  if (!property) {
    return (
      <section className={`bg-card border border-border rounded-xl shadow-sm p-6 ${className ?? ''}`}>
        <h2 className="text-lg font-semibold text-foreground mb-4">Propiedad</h2>
        <div className="text-center py-8">
          ...
          <div className="text-sm font-medium text-foreground mt-2">No hay información de propiedad</div>
          <div className="text-sm text-muted-foreground mt-1">La información de la propiedad aparecerá aquí cuando esté disponible</div>
        </div>
      </section>
    );
  }
          ...
          {property.images && property.images.length > 1 && (
            <div className="flex gap-2 overflow-x-auto pb-2">
              {property.images.slice(0, 5).map((image, index) => (
                <button
                  ...
                  className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 ${
                    selectedImageIndex === index ? 'border-brand-sky' : 'border-border'
                  }`}
                >
                  <img
                    src={image}
              ...
              {property.images.length > 5 && (
                <div className="flex-shrink-0 w-16 h-16 bg-muted rounded-lg flex items-center justify-center text-xs text-muted-foreground border-2 border-border">
                  +{property.images.length - 5}
                </div>
              )}
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-muted/30 rounded-lg p-3">
              ...
              <div className="text-lg font-semibold text-foreground">{getPropertyTypeLabel(property.property_type)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="overview" className="space-y-4 mt-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
              <span className="text-sm text-muted-foreground">Parqueaderos</span>
              <span className="font-semibold text-foreground">{property.parking_spaces || property.parking || 0}</span>
            </div>
            {property.year_built && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Año construcción</span>
            ...
            {property.strata_fee && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                ...
                <span className="font-semibold text-foreground">{formatPrice(property.strata_fee)}</span>
              </div>
            )}
            {property.monthly_costs && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Costos mensuales</span>
                <span className="font-semibold text-foreground">{formatPrice(property.monthly_costs)}</span>
              </div>
            )}
          </div>
        ...
        <TabsContent value="documents" className="mt-4">
          {property.legal_documents && property.legal_documents.length > 0 ? (
            ...
            <div className="text-center py-8">
              <FileText className="mx-auto h-12 w-12 text-muted-foreground/70 mb-2" />
              <div className="text-sm font-medium text-foreground">No hay documentos</div>
              <div className="text-sm text-muted-foreground mt-1">Los documentos legales aparecerán aquí cuando estén disponibles</div>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="technical" className="mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {property.total_floors && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Total de pisos</span>
                <span className="font-semibold text-foreground">{property.total_floors}</span>
              </div>
            )}
            {property.strata && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Estrato</span>
                <span className="font-semibold text-foreground">{property.strata}</span>
              </div>
            )}
            {property.financing !== null && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Financiación</span>
                <span className="font-semibold text-foreground">{property.financing ? 'Sí' : 'No'}</span>
              </div>
            )}
            {property.accepts_crypto !== null && (
              <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                <span className="text-sm text-muted-foreground">Acepta cripto</span>
                <span className="font-semibold text-foreground">{property.accepts_crypto ? 'Sí' : 'No'}</span>
              </div>
            )}
